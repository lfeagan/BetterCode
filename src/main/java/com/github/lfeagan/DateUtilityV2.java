...
public class DateUtilityV2 {
    ...
    private static ThreadLocal<SimpleDateFormat> threadLocalRfc3339DateFormat = ThreadLocal.withInitial(() -> new SimpleDateFormat(RFC3339_DATE_FORMAT_STRING));
    ...
    public static Date formatDate(String dateString) throws ParseException {
        Date yearMonthDate = threadLocalRfc3339DateFormat.get().parse(dateString);

        Calendar cal = Calendar.getInstance();
        cal.setTime(yearMonthDate);
        return Date.from(ZonedDateTime.of(cal.get(Calendar.YEAR), cal.get(Calendar.MONTH) + 1, cal.get(Calendar.DAY_OF_MONTH),
                cal.get(Calendar.HOUR_OF_DAY), cal.get(Calendar.MINUTE), cal.get(Calendar.SECOND), 0,
    ...
    public static Date frameQueryForDateForMobile(String date, String key) {
        ...
        if (date.indexOf(":") > 0) {
            ...
            monthNumber += 1;
            return new DateTime(year, monthNumber, dateNumber, cal.get(Calendar.HOUR_OF_DAY),
            ...
            if (key.equalsIgnoreCase("from_date")) {
                return new DateTime(year, monthNumber, dateNumber, 0, 0, DateTimeZone.UTC)
                        .toDate();
            } else if (key.equalsIgnoreCase("to_date")) {
                return new DateTime(year, monthNumber, dateNumber, 23, 59, DateTimeZone.UTC)
                        .toDate();
            } else {
    ...
    public static Date removeMinutesFromDate(String date) {
        ...
        int dateNumber = cal.get(Calendar.DAY_OF_MONTH);
        monthNumber += 1;
        return new DateTime(year, monthNumber, dateNumber, cal.get(Calendar.HOUR_OF_DAY), 0, 0,
    ...
    public static Date removeTimeFromDate(String date) {
        ...
        int dateNumber = cal.get(Calendar.DAY_OF_MONTH);
        monthNumber += 1;
        return new DateTime(year, monthNumber, dateNumber, 0, 0, 0, DateTimeZone.UTC).toDate();
    }
